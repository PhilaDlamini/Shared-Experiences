...
int List_contains(List list, char *clientId) {
    for(int i = 0; i < list->size; i++) {
...
/*Bad idea
void List_closeInvalidFiles(List list) {

    for(int i = 0; i < list->size; i++) {
        Entry entry = list->data[i];
        printf("checking for %d\n", entry->fd);

        if(fcntl(entry->fd, F_GETFD) == -1) {
            List_remove(list, entry->clientID);
            close(entry->fd);
            printf("Found invalid fd from %s. Closing connection\n", entry->clientID);
        }
    }
}*/